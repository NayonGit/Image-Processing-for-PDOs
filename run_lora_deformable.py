#!/usr/bin/env python3
"""
Script CLI for training and testing object detection models with LoRA and a Deformable DETR head.

Usage:
    # Train a model
    python3 run_lora_deformable.py train --dataset-name orgaquant --batch-size 8 --max-epochs 100
...
def get_train_parser():
    ...
    parser = argparse.ArgumentParser(description="Train a Deformable DETR detection model with LoRA")
    
    # Data
    parser.add_argument("--dataset-name", type=str, default="orgaquant",
    ...
    parser.add_argument("--num-feature-levels", type=int, default=1,
                        help="Number of feature levels for Deformable DETR (only 1 is supported with DINO backbone)")
    ...
    parser.add_argument("--no-patching", dest="use_patching", action="store_false",
                        help="Disable patching")
...
def get_test_parser():
    ...
    parser.add_argument("--dec-layers", type=int, default=6,
                        help="Number of transformer decoder layers")
    parser.add_argument("--num-feature-levels", type=int, default=1,
                        help="Number of feature levels for Deformable DETR")
...
def main():
    ...
    root_parser = argparse.ArgumentParser(
        description="Training and testing script for Deformable DETR with LoRA",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train a model
  python run_lora_deformable.py train --dataset-name orgaquant --batch-size 8

  # Resume training from checkpoint
  python run_lora_deformable.py train --dataset-name orgaquant --resume-from ./checkpoints/last.ckpt

  # Test a model
  python run_lora_deformable.py test --ckpt-path ./checkpoints/best.ckpt --dataset-name orgaquant
        """
    )
    ...
    if args.command == "train":
        print("=" * 80)
        print("TRAINING DEFORMABLE DETR MODEL")
        print("=" * 80)
