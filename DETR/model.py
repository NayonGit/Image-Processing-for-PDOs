...
from DETR.lora import create_backbone_with_lora
from DETR.detr_utils import SetCriterion, HungarianMatcher, TransformerDetectionHead
from DETR.utils import box_cxcywh_to_xyxy, extract_patch_targets, get_boundaries
from DETR.deformable_detr import build_deformable_detection_head


"""
This file defines the LoRADetectionModel class, which is a PyTorch Lightning module for object detection using a backbone with PEFT LoRA adapters and a DETR detection head.
The model supports optional patch-based training, where images are divided into overlapping patches and the model is trained on these patches instead of full images. 
The training and validation steps are custom implemented to handle both full-image and patch-based training, and mAP metrics are computed at the end of each validation epoch.
"""
# =============================================================================
# Full Detection Model with Patching (Lightning Module)
# =============================================================================

class LoRADetectionModel(L.LightningModule):
    """
    Object Detection model with PEFT LoRA on the backbone and DETR Detection Head.
    """
    ...
    def __init__(
        ...
        self.val_metric = torchmetrics.detection.MeanAveragePrecision(iou_type="bbox") #.to(self.device)

        if use_patching:
            #self.boundaries, self.img_size = get_boundaries(num_patches, overlap_size, patch_size)
        ...
        # Backbone with LoRA adapters (frozen except for LoRA layers)
        ...
        if deformable_attention:
            self.detection_head = build_deformable_detection_head(
    ...
    def forward_patch(self, patch: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # dino_features shape: [B, N_patches, C]
        dino_features = self.backbone.extract_features(patch)
        bs, n_patches, feat_dim = dino_features.shape
        if self.hparams.deformable_attention:
            h_patches = w_patches = int(n_patches**0.5)
            src = dino_features.permute(0, 2, 1).reshape(bs, feat_dim, h_patches, w_patches)

            if hasattr(self, 'input_proj'):
                src = self.input_proj[0](src)
            b, c, h, w = src.shape
            memory = src.reshape(b, c, h * w).permute(2, 0, 1)
            predictions = self.detection_head(memory)
        else:
            # [B, N, C] -> [N, B, C]
            memory = dino_features.permute(1, 0, 2)
            predictions = self.detection_head(memory)
        return predictions
    ...
    def configure_optimizers(self):
        """Configure optimizers with separate learning rates for LoRA parameters and detection head parameters."""
        # PEFT models expose trainable params via named_parameters, we can separate them based on module names
        lora_params = []
        head_params = []
