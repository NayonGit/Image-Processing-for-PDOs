...
"""
This file contains the training and testing scripts for the DETR model with PEFT LoRA on H5 datasets.
The `train` function handles the full training pipeline, including dataset loading, model creation with LoRA, training loop with early stopping and checkpointing, and final testing on the validation set.
The `test` function allows testing a trained model on the validation set using a specified checkpoint.
Both functions support configuration of the dataset, model architecture, LoRA parameters, training hyperparameters, and patching options.
"""
...
def train(
    ...
    Args:
        resume_from_checkpoint: Path to a checkpoint file to resume training from.
                               If provided, will load the model and its training state
