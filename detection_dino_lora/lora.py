...
def get_lora_target_modules(backbone_name: str, target_all: bool = False) -> List[str]:
    ...
    if "dino" in backbone_name:
        return ["qkv", "proj", "fc1", "fc2"] if target_all else ["qkv"]
    raise ValueError(f"Unknown backbone for LoRA target modules: {backbone_name}")
...
def create_backbone_with_lora(
    ...
    """
    if name == "dinov2":
        hub_name, num_features = DINOV2_SIZE_MAP[model_size]
        model = torch.hub.load("facebookresearch/dinov2", hub_name)
    elif name == "dinov3":
        hub_name, num_features = DINOV3_SIZE_MAP[model_size]
        files = glob.glob(os.path.join(WEIGHTS_DIR, f"{hub_name}.pth"))
        if not files:
            raise ValueError(f"No weights file found for {hub_name} in {WEIGHTS_DIR}")
        model = torch.hub.load(
            REPO_DIR,
            hub_name,
            source="local",
            weights=os.path.abspath(files[0]),
        )
    else:
    ...
    # DINOv2/3 token extractor (patch tokens)
    def extract_features(self, x: torch.Tensor) -> torch.Tensor:
        out = self.forward(x, is_training=True)
        return out["x_norm_patchtokens"]  # [B, N, C]
